...
/** Grok `/effort` accepts low | medium | high | xhigh. Map Claude-only tiers down. */
function grokEffortValue(effort?: EffortLevel): string | undefined {
  if (!effort) return undefined;
  if (effort === 'max' || effort === 'ultra') return 'xhigh';
  return effort;
}

/** Global `grok` flags (before the `agent` subcommand). */
function globalArgs(opts: GrokAcpRunOptions): string[] {
  const args: string[] = ['--no-auto-update'];
  // `--permission-mode` is a top-level grok flag, not an `agent` flag.
  if (opts.permissionMode === 'plan') args.push('--permission-mode', 'plan');
  return args;
}

/**
 * Flags that belong on `grok agent … stdio` — after `agent`, before `stdio`.
 * `--always-approve` before `agent` is parsed as the TUI flag and ignored.
 */
function agentArgs(opts: GrokAcpRunOptions): string[] {
  const args: string[] = [];
  if (opts.permissionMode === 'bypassPermissions') args.push('--always-approve');
  if (opts.model && opts.model !== 'auto') args.push('--model', opts.model);
  const effort = grokEffortValue(opts.effort);
  if (effort) args.push('--effort', effort);
  return args;
}

function sessionMeta(opts: GrokAcpRunOptions): Record<string, unknown> | undefined {
  if (opts.permissionMode === 'bypassPermissions') return { yoloMode: true };
  if (opts.permissionMode === 'acceptEdits') return { autoMode: true };
  return undefined;
}

function buildAcpSpawn(opts: GrokAcpRunOptions): { bin?: string; args: string[]; remote: boolean; cwd?: string } {
  const args = [...globalArgs(opts), 'agent', ...agentArgs(opts), 'stdio'];
  if (!opts.remote) return { bin: config.grokExecutable, args, remote: false, cwd: opts.cwd };
...
/** One Grok ACP process for one turn (initialize → auth → new/load → configure → prompt). */
export class GrokAcpClient {
  ...
  async run(): Promise<{ error?: string }> {
    ...
    if (!spawnSpec.bin) return { error: 'grok not found — install Grok Build or set GROK_CLI_PATH' };
    ...
    const exitPromise = new Promise<void>((resolve) => {
      ...
      this.child!.on('close', (code) => {
        this.closed = true;
        this.rejectAll(new Error(cleanRemoteStderr(this.stderr) || `grok agent stdio exited with code ${code}`));
        resolve();
      });
    });
  ...
  private async authenticate(init: any): Promise<void> {
    const methods: string[] = Array.isArray(init?.authMethods)
      ? init.authMethods.map((m: any) => String(m?.id ?? '')).filter(Boolean)
      : [];
    if (!methods.length) return;
    const hasKey = Boolean(process.env.XAI_API_KEY);
    const methodId = hasKey && methods.includes('xai.api_key')
      ? 'xai.api_key'
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      if (/already authenticated|not required/i.test(message)) return;
      throw error;
    }
  }
  ...
  private async openSession(cwd: string): Promise<string> {
    ...
    const _meta = sessionMeta(this.opts);
    if (this.opts.resume) {
      try {
        this.ignoreUpdates = true;
        await this.request('session/load', { sessionId: this.opts.resume, cwd, mcpServers, _meta });
        this.ignoreUpdates = false;
        return this.opts.resume;
      } catch (error) {
        this.ignoreUpdates = false;
        log.warn('grok acp session/load failed, starting new session', error);
      }
    }
    const created = await this.request('session/new', { cwd, mcpServers, _meta });
  ...
  private async applySessionConfig(): Promise<void> {
    ...
    if (this.opts.model && this.opts.model !== 'auto') {
      try {
        ...
        log.debug('grok acp set_model failed', this.opts.model, error);
      }
    }
    const effort = grokEffortValue(this.opts.effort);
    if (effort) {
      try {
  ...
  private async dispatch(message: any): Promise<void> {
    ...
    if (message.method === 'session/update' || message.method.startsWith('_x.ai/')) {
      if (!this.ignoreUpdates && message.method === 'session/update') this.handleUpdate(message.params?.update);
    ...
    try {
      ...
      log.warn('grok acp handler failed', message.method, error);
  ...
  private async handleRequestPermission(params: any): Promise<unknown> {
    ...
    // Always-approve: never surface a prompt even if Grok still asks (resume,
    // leftover ask-rules, or a session that was created before yolo was set).
  ...
  private segment(kind: 'assistant' | 'thinking', text: string): void {
    ...
    if (!this.stream) {
      const id = `grok_acp_${crypto.randomUUID()}`;
